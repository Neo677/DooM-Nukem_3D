...
int intersect_segments(t_v2 p1, t_v2 p2, t_v2 p3, t_v2 p4, t_v2 *intersection)
{
    ...
    // Si r x s = 0, lignes parallèles
    if (fabs(r_cross_s) < 1e-6)
    {
        // Colinéaires si (q-p) x r = 0
        if (fabs(qp_cross_r) < 1e-6)
        {
            // Vérifier chevauchement (plus complexe, ignoré pour l'instant)
            return (0); 
        }
        return (0); // Parallèles non colinéaires
    }
...
// Ici : méthode générique pour tout polygone simple
...
// Vérifie si un polygone est convexe (tous les cross products de même signe)
